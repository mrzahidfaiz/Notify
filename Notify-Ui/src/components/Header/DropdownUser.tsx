...
import { logout, undoImpersonationApi } from "../../api/api";
...
const DropdownUser = () => {
  ...
  const [showChangePasswordModel, setShowChangePasswordModel] =
  ...
  let userName;
  let userEmail;
  let impersonatedUser: string = "";
  if (user) {
    const { company_name, email, name, impId } = JSON.parse(user);
    impersonatedUser = impId;
  ...
  async function undoImpersonation() {
    const res: any = await undoImpersonationApi(impersonatedUser);
    let name;
  ...
  return (
    <div className="relative">
      {showChangePasswordModel ? (
        <ChangePassword setShowChangePassword={setShowChangePasswordModel} />
      ) : null}
      <span
        ref={trigger}
      ...
      >
        <div className="flex flex-col items-center gap-4 py-2 text-sm font-medium duration-300 ease-in-out lg:text-base">
          <span>
